...
while True:
    ...
    with mp_pose.Pose(
        ...
        # Convert the OpenCV BGR image to Pillow Image
        pillow_image = Image.fromarray(processed_frame)

        # Display the frame in Streamlit for shape detection
        webcam_placeholder.image(pillow_image)

        # Convert the OpenCV BGR image to Pillow Image
        pillow_image_pose = Image.fromarray(frame)

        # Display the frame in Streamlit for pose detection
        webcam_placeholder.image(pillow_image_pose)
